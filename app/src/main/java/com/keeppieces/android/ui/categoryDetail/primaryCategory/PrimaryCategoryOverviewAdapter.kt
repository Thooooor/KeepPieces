...
class PrimaryCategoryOverviewAdapter(val primarySummary: List<DailyPrimary>,val primaryIncome:Double,val primaryExpenditure:Double):
    ...
    inner class ViewHolder(view: View):RecyclerView.ViewHolder(view) {
        ...
        val cardVerticalBar:com.keeppieces.line_indicator.VerticalBar=view.findViewById(R.id.cardVerticalBar)
    ...
    fun getDesciptionOfThePrimary(onePrimaryBillList:DailyPrimary):List<Pair<String,String>>{
        ...
        if (abs(primaryIncome - 0)>0.01  && abs(onePrimaryBillList.income - 0)>0.01) {
            description.add(Pair("${onePrimaryBillList.primaryCategory}收入(占总收入比)：",
                onePrimaryBillList.income.toCHINADFormatted()+"("+String.format("%.2f",onePrimaryBillList.income/primaryIncome*100)+"%)"))
        }
        if (abs(primaryExpenditure - 0)>0.01  && abs(onePrimaryBillList.expenditure - 0)>0.01) {
            description.add(Pair("${onePrimaryBillList.primaryCategory}支出(占总支出比)：",
                onePrimaryBillList.expenditure.toCHINADFormatted()+"("+String.format("%.2f",onePrimaryBillList.expenditure/primaryExpenditure*100)+"%)"))
        }
        description.add(Pair("${onePrimaryBillList.primaryCategory}下的二级类个数:",onePrimaryBillList.secondaryNum.toString()))
        return description
    }
}
